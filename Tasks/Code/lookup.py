...
class WikidataAPI(Lookup):
    ...
    def __extractKGEntities(self, json, filter=''):
        ...
        for element in json['search']:
            ...
            types = self.getTypesForEntity(element['concepturi'])
    ...
    def getTypesForEntity(self, entity_id):
    # API request to get types  
        params = {
        ...
        types = set()
        res = self.getJSONRequest(params)
        if res:
            data = res['entities'][entity_id]  
            if 'claims' in data:
                claims = data['claims']
                if 'P31' in claims:  # assuming 'P31' is the key for the type information
                    for c in claims['P31']:
                        types.add(c['mainsnak']['datavalue']['value']['id'])
